...
# Locus affects directly both trait 1 and trait 2, no direct effect from trait 1 to trait 2 and vice-versa
...
#regression
fit1<-lm(y1~1+y2, data=data)
summary(fit1)

fit2<-lm(y1~1+geno+y2, data=data)
summary(fit2)

cor(fitted(fit1), data$y1)
cor(fitted(fit2), data$y1)
